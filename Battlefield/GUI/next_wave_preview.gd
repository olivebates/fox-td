...
func _process(delta: float) -> void:
	...
	if _poll_timer < POLL_INTERVAL:
		return
	_poll_timer = 0.0
	var wave_index = WaveSpawner.get_next_wave_index()
	var display_wave = _get_display_wave_index(wave_index)
	if wave_index > WaveSpawner.MAX_WAVES:
		text = "Final: " + str(int(display_wave))
		return
	text = "Next: " + str(int(display_wave))
...
func _on_mouse_entered() -> void:
	...
	var title_prefix = "Final Wave " if wave_index > WaveSpawner.MAX_WAVES else "Wave "
