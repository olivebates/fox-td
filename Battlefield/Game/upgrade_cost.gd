...
func _process(delta: float) -> void:
	...
	if parent_node == null or !is_instance_valid(parent_node):
		visible = false
		return
	if not parent_node.has_meta("item_data"):
		visible = false
		var sprite = parent_node.get("sprite")
	...
	var upgrade_button = get_tree().get_first_node_in_group("upgrade_button")
	var upgrade_mode_active = upgrade_button != null and upgrade_button.upgrade_mode
	...
	if is_matching:
		...
		var sprite = parent_node.get("sprite")
		if sprite != null and is_instance_valid(sprite):
			sprite.modulate = Color(2.0, 0.0, 0.0) if not can_afford else Color(1, 1, 1)
	elif upgrade_mode_active:
		var cost = InventoryManager.get_placement_cost(data.id, 0, rank + 1)
